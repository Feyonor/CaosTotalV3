...
public class ChaosLighter extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient && player != null) {
            ...
            if (isPortalValid(world, pos)) {
                ...
                String currentDim = world.getDimensionKey().getValue().toString();
                ...
                player.sendMessage(Text.literal("§c❌ Necesita un portal de bloques de diamante (3x3)"), false);
            }
        }
    ...
    private boolean isPortalValid(World world, BlockPos pos) {
        // Verificar si hay un patrón de portal de diamante (3x3)
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
    ...
    private void teleportToChaos(PlayerEntity player) {
        if (player instanceof net.minecraft.server.network.ServerPlayerEntity serverPlayer) {
            ServerWorld chaosWorld = serverPlayer.getServer().getWorld(new Identifier("caostaotal", "chaos"));
            if (chaosWorld != null) {
                ...
                serverPlayer.sendMessage(Text.literal("§c⚡ ¡¡¡PREPÁRATE PARA EL CAOS!!!"), false);
            } else {
                player.sendMessage(Text.literal("§c❌ La dimensión del Caos no existe. Contáctalo al admin."), false);
            }
        }
    }
    ...
    private void teleportToOverworld(PlayerEntity player) {
        if (player instanceof net.minecraft.server.network.ServerPlayerEntity serverPlayer) {
            ServerWorld overworldWorld = serverPlayer.getServer().getWorld(new Identifier("minecraft", "overworld"));
            if (overworldWorld != null) {
                serverPlayer.teleport(overworldWorld, 0, 100, 0, 0, 0);
                serverPlayer.sendMessage(Text.literal("§aHas escapado del Caos... por ahora"), false);
            }
        }
    }
}
